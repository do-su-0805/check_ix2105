...
public_ip=`snmpwalk -v 2c -c localreader 192.168.1.1 RFC1213-MIB::ipAdEntAddr | grep -v 192 | awk '{print $NF}'`
...
if [ ${public_ip} != ${before_public_ip} ]; then
    ...
    curl -s -X PUT ${METADATA_POINT} -H "X-Api-Key:${API_KEY}" -H 'Content-Type: application/json' -d $data
else
